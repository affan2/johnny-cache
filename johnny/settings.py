...
BLACKLIST = list(getattr(settings, 'MAN_IN_BLACKLIST', getattr(settings, 'JOHNNY_TABLE_BLACKLIST', []))) + DEFAULT_BLACKLIST
BLACKLIST = set(BLACKLIST)

WHITELIST = set(getattr(settings, 'JOHNNY_TABLE_WHITELIST', []))

DB_CACHE_KEYS = dict((name, db.get('JOHNNY_CACHE_KEY', name)) for name, db in list(settings.DATABASES.items()))
...
CACHE_BACKEND = getattr(settings, 'JOHNNY_CACHE_BACKEND', getattr(settings, 'CACHE_BACKEND', None))
